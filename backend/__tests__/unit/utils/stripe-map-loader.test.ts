/**
 * Stripe map loader – getStripeMap, getStripeTierConfig, getStripePriceId,
...
describe("stripe-map-loader", () => {
  ...
  describe("getStripePriceAmount", () => {
    test("returns monthly amount for basic", () => {
      expect(getStripePriceAmount("basic", "monthly")).toBe(19);
    });

    test("returns annual amount for basic", () => {
      expect(getStripePriceAmount("basic", "annual")).toBe(190);
    });
