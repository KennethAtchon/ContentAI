...
const PRODUCTS = {
  basic:      { id: "prod_TWTXj1UeJcW6vz", name: "Creator",  description: "For individual creators getting started with AI content." },
  pro:        { id: "prod_TWTYPXmd7zh3kP", name: "Pro",       description: "For creators who want full AI generation and publishing." },
  enterprise: { id: "prod_TWTYPkmPHd8GF4", name: "Agency",   description: "For agencies and power users who need unlimited scale." },
} as const;

const OLD_PRICES = {
  basic:      { monthly: "price_1SZQa63qLZiOfTxsQZkBift7", annual: "price_1SZQak3qLZiOfTxsM7kwhZwQ" },
  pro:        { monthly: "price_1SZQaz3qLZiOfTxs8Kg7ZsN8", annual: "price_1SZQbE3qLZiOfTxsx4kE2xqk" },
  enterprise: { monthly: "price_1SZQbQ3qLZiOfTxsIye7eUZm", annual: "price_1SZQbe3qLZiOfTxs7xApSWQY" },
} as const;
...
async function run() {
  ...
  const newPriceIds: Record<TierKey, { monthly: string; annual: string }> = {} as any;

  for (const [tierKey, product] of Object.entries(PRODUCTS) as [TierKey, typeof PRODUCTS[TierKey]][]) {
    ...
    console.log(`  Creating monthly price $${NEW_AMOUNTS[tierKey].monthly / 100}...`);
    ...
    console.log(`  Creating annual price $${NEW_AMOUNTS[tierKey].annual / 100}...`);
    ...
    console.log(`  Archiving old monthly price ${OLD_PRICES[tierKey].monthly}...`);
    await stripe.prices.update(OLD_PRICES[tierKey].monthly, { active: false });
    console.log(`  ✓ Old monthly price archived`);

    console.log(`  Archiving old annual price ${OLD_PRICES[tierKey].annual}...`);
  ...
  for (const [tierKey, ids] of Object.entries(newPriceIds) as [TierKey, { monthly: string; annual: string }][]) {
    const product = PRODUCTS[tierKey];
    const amounts = NEW_AMOUNTS[tierKey];
    console.log(`${product.name} (${tierKey}):`);
    console.log(`  monthly: { priceId: "${ids.monthly}", amount: ${amounts.monthly / 100} }`);
    console.log(`  annual:  { priceId: "${ids.annual}",  amount: ${amounts.annual / 100} }`);
    console.log();
  }
