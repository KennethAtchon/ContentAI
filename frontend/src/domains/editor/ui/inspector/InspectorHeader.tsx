...
const TRACK_TYPE_BADGE: Record<string, { label: string; className: string }> = {
  video: {
    label: "VIDEO",
...
export function InspectorHeader({
  ...
  const badge = selectedTransition
    ...
    : selectedTrack
      ? (TRACK_TYPE_BADGE[selectedTrack.type] ?? {
          label: selectedTrack.type.toUpperCase(),
          className: "bg-overlay-md text-dim-2 border-overlay-md",
        })
      : null;
    ...
    <div className="shrink-0 border-b border-overlay-sm">
      {badge && (
        <div className="flex items-center gap-2 px-3 pt-2.5 pb-1.5">
          ...
          {selectedClipLabel && (
            <span className="text-xs text-dim-2 truncate">
              {selectedClipLabel}
            </span>
          )}
        </div>
