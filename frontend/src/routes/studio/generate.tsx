...
function GeneratePage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await generateContent.mutateAsync({ sourceReelId: selectedReelId, prompt, outputType });
  ...
  return (
    <AuthGuard authType="user">
      <div className="h-screen bg-studio-bg text-studio-fg font-studio grid grid-rows-[48px_1fr] overflow-hidden">
        <StudioTopBar niche={inputNiche} onNicheChange={setInputNiche} onScan={() => setNiche(inputNiche)} activeTab="generate" />
        ...
        <div className="grid overflow-hidden" style={{ gridTemplateColumns: "280px 1fr 320px" }}>
            ...
            <div className="overflow-y-auto flex-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {reels.map((reel) => {
                ...
                return (
                  ...
                  >
                    <div className={cn("w-[34px] h-[34px] rounded-[6px] flex items-center justify-center text-base shrink-0",
                      isActive ? "bg-studio-accent/15" : "bg-white/[0.06]",
                    )}>
                      {reel.thumbnailEmoji ?? "🎬"}
                    </div>
                    <div className="min-w-0">
                      <p className="text-[12px] font-semibold text-studio-fg truncate">{reel.username}</p>
                      <p className="text-[10px] text-slate-200/35 mt-px">
                        {reel.hasAnalysis ? <span className="text-studio-accent">✦ Analyzed</span> : "Not analyzed"}
                      </p>
                    </div>
                  </button>
                );
            ...
            <div className="max-w-[560px] mx-auto w-full">
              ...
              <div className="mt-3">
                <button
                  ...
                  disabled={generateContent.isPending || !prompt.trim() || !selectedReelId}
                  className={cn(
                ...
                {generateContent.isError && (
                  <p className="text-[11px] text-red-400 mt-2">{t("studio_generate_error")}</p>
                )}
              </div>
                ...
                <div className="mt-5 bg-studio-accent/[0.08] border border-studio-accent/20 rounded-xl p-4">
                  ...
                  {result.generatedScript && (
                    <ul className="list-disc pl-5 space-y-1 mb-3">
                      {(JSON.parse(result.generatedScript) as string[]).map((note, i) => (
                        <li key={i} className="text-[11px] text-slate-200/50">{note}</li>
                      ))}
                    </ul>
                  )}
                  <div className="flex gap-2 flex-wrap">
                    {result.generatedHook && (
                      <CopyBtn onClick={() => navigator.clipboard.writeText(result.generatedHook!)}>Copy Hook</CopyBtn>
                    )}
                    {result.generatedCaption && (
                      <CopyBtn onClick={() => navigator.clipboard.writeText(result.generatedCaption!)}>Copy Caption</CopyBtn>
                    )}
                    <button
                      onClick={async () => { await queueContent.mutateAsync({ contentId: result.id }); setResult(null); }}
          ...
          <aside className="bg-studio-surface border-l border-white/[0.05] flex flex-col overflow-hidden">
            <div className="flex items-center justify-center px-4 py-2.5 border-b border-white/[0.05] shrink-0">
              <span className="text-[11px] font-semibold text-studio-accent">{t("studio_panel_history")}</span>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {history.length === 0 ? (
                <div className="flex flex-col items-center gap-3 py-10 text-center">
                  ...
                  <p className="text-[13px] font-semibold text-slate-200/40">{t("studio_history_empty")}</p>
                </div>
              ) : (
                history.map((item) => (
                  <div key={item.id} className="bg-white/[0.03] border border-white/[0.06] rounded-[10px] p-3 cursor-pointer transition-colors hover:border-studio-accent/30">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-200/30">
                      <span className={cn("text-[9px] font-bold px-1.5 py-px rounded-full uppercase tracking-[0.5px]", STATUS_STYLES[item.status] ?? STATUS_STYLES.draft)}>
                        {item.status}
                      </span>
...
function CopyBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className="text-[11px] font-semibold px-3 py-1.5 rounded-lg border border-white/10 bg-white/[0.05] text-slate-200/60 cursor-pointer font-studio transition-all hover:bg-white/10 hover:text-studio-fg">
      {children}
    </button>
  );
}
