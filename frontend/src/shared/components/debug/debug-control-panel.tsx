...
export function DebugControlPanel() {
  ...
  const shouldRender = typeof window !== "undefined" && (IS_DEVELOPMENT || DEBUG_ENABLED);
      ...
      {!isOpen ? (
        ...
        <div className="w-[430px] max-w-[90vw] rounded border border-stroke bg-black p-3 shadow-2xl">
          ...
          <div className="mb-2 rounded border border-stroke p-2">
            ...
            <div className="mb-2 flex flex-wrap gap-1">
              {knownNamespaces.map((namespace) => {
                const isEnabled = !snapshot.disabledNamespaces.includes(namespace);
                return (
                  <button
                    key={namespace}
                    type="button"
                    onClick={() => debugLog.setNamespaceEnabled(namespace, !isEnabled)}
          ...
          <div className="mb-2 flex items-center justify-between gap-2 text-dim-3">
            <span>
              Stream entries: {snapshot.streamEntryCount} / {snapshot.streamLimit}
            </span>
