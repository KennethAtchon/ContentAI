  ...
  return (
    ...
    <div>
      ...
      <div className="px-3 py-3 space-y-3">
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-1.5 rounded border border-overlay-md bg-overlay-xs text-sm font-medium text-dim-2 hover:text-dim-1 hover:bg-overlay-sm disabled:opacity-40 transition-colors"
          >
          ...
          <button
            ...
            className="flex-1 py-1.5 rounded border border-success/30 bg-success/10 text-sm font-semibold text-success/80 hover:bg-success/18 hover:text-success disabled:opacity-40 transition-colors inline-flex items-center justify-center gap-1.5"
          >
        ...
        {jobStatus === "failed" && (
          <button
            ...
            className="w-full py-1.5 rounded border border-error/25 bg-error/8 text-sm font-medium text-error/70 hover:bg-error/15 disabled:opacity-40 transition-colors"
          >
        ...
        {/* Job progress */}
        {compositionJob.data && (
          <div className="space-y-2 text-sm text-dim-2">
      ...
      <div className="border-t border-overlay-sm px-3 py-2.5">
        <p className="text-sm font-bold uppercase tracking-widest text-dim-3 mb-2">
          {t("phase5_editor_versions")}
        </p>
        <div className="space-y-1">
          {versions.data?.items.length ? (
            versions.data.items.slice(0, 3).map((item) => (
              <p key={item.assetId} className="text-sm text-dim-3 truncate">
                {item.label}
              </p>
            ))
          ) : (
            <p className="text-sm text-dim-3">{t("phase5_editor_versions_empty")}</p>
          )}
        </div>
      </div>
    </div>
