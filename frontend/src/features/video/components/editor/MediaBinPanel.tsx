  ...
  const renderVideoGroup = (
    ...
    return (
      <div className="space-y-px">
        <p className="px-3 py-1.5 text-sm font-bold uppercase tracking-widest text-dim-3">
          {title}
        </p>
        {groupAssets.slice(0, 12).map((asset) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="truncate text-sm text-dim-1 font-medium leading-tight">
                {asset.id.slice(-8)}
              </p>
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
              <button
                ...
                className="px-1.5 py-0.5 rounded text-sm font-semibold text-blue-300/70 hover:text-blue-200 hover:bg-blue-500/20 transition-colors"
              ...
              <button
                ...
                className="px-1.5 py-0.5 rounded text-sm font-semibold text-dim-2 hover:text-dim-1 hover:bg-overlay-sm transition-colors"
    ...
    <div className="flex flex-col h-full">
      ...
      {isLoading ? (
        ...
        <div className="flex-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {/* Video section */}
          <div className="py-1">
            <p className="px-3 py-1.5 text-sm font-semibold text-dim-2">
            ...
            {videoAssets.length > 0 ? (
              ...
              <div className="px-3 py-4 space-y-2">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <a
                    href="/studio"
                    className="text-sm text-dim-2 hover:text-dim-1 transition-colors"
                  >
              ...
              audioAssets.slice(0, 8).map((asset) => (
                ...
                >
                  <div className="w-9 h-6 rounded bg-purple-500/15 border border-overlay-md shrink-0 flex items-center justify-center">
                    <span className="text-sm text-purple-300/40">♫</span>
                  </div>
                  <p className="truncate text-sm text-dim-2">{asset.id.slice(-8)}</p>
                </div>
              ))
            ) : (
              <p className="px-3 py-2 text-sm text-dim-3">
