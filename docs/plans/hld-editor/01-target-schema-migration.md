...
- PostgreSQL JSONB guidance: [https://www.postgresql.org/docs/current/datatype-json.html](https://www.postgresql.org/docs/current/datatype-json.html)
- W3C WGSL shader lifecycle: [https://www.w3.org/TR/WGSL/#shader-lifecycle](https://www.w3.org/TR/WGSL/#shader-lifecycle)
- W3C WebVTT cue model: [https://www.w3.org/TR/webvtt1/#webvtt-cues](https://www.w3.org/TR/webvtt1/#webvtt-cues)
- OpenTimelineIO timeline/media reference model: [https://opentimelineio.readthedocs.io/en/v0.16.0/tutorials/otio-timeline-structure.html](https://opentimelineio.readthedocs.io/en/v0.16.0/tutorials/otio-timeline-structure.html)
...
- Whether `edit_project_artifact` is created in Phase 1 even if no migration artifacts will be written (Phase 2 skipped).
...
1. Add new columns/tables as nullable where a staging window is needed.
2. Create indexes and constraints that do not depend on converted data.
3. ~~Run Phase 2 conversion~~ **Skipped** — DB reset; new projects write `project_document` directly.
4. Add non-null constraints on new rows as runtime code enforces them.
...
- Local migration applies to an empty database (Phase 2 skipped; no legacy rows to test against).
- New project rows written by runtime have non-null `project_document`, `project_document_version`, `editor_core_version`, `document_hash`, and `save_revision`.
- ~~Every converted project has an initial `cutover_import` revision.~~ Not applicable — no legacy data.
...
- ~~Phase 2 has enough schema surface to write converted documents, initial revisions, asset links, and migration artifacts.~~ Not applicable — Phase 2 skipped.
