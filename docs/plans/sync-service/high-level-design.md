...
**Chat/AI system** - the user talks to the model, which generates and iterates `generated_content` rows (hook, script, voiceover, caption). Those rows are now attached to chat sessions through `chat_session_content`, and `chat_sessions.activeContentId` is the server-side pointer for the draft the user is currently iterating.
...
The chat session now keeps drafts and active-draft state coherent on its own. SyncService's job is narrower: whenever a text content write succeeds, make sure every linked editor project catches up to that saved content before the client finishes processing the stream.
...
These tools run synchronously inside the AI chat stream. When they complete, the chat layer has already written the content row and updated the current session's draft membership / active draft state. SyncService can then re-derive the full editor timeline immediately and write it to the DB before the SSE event fires to the frontend.
...
SyncService never bumps any version counter. It writes tracks and denormalized text fields - that is it. If the editor autosaves after sync, the editor's payload wins on whatever fields it sends. This is intentional: the user's manual edits are never discarded by AI activity.
...
When `iterate_content` creates a new content version, that version has no video assets yet. `deriveTimeline` would return an empty video track. Rather than clearing the editor's video track (which would be destructive and confusing), SyncService preserves the existing video clips unchanged when the fresh video track is empty. The user still has a working video; it just does not match the new script until they regenerate.
...
Session membership is the source of truth for which drafts belong to the current chat workspace. `edit_projects.generatedContentId` is still advanced to the latest content version by each sync. The ancestor chain (`resolveContentAncestorChainIds`) lets SyncService find editor projects linked to any version in the lineage.
