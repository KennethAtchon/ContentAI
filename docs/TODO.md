...
The right direction is a discriminated union:
  type TimelineClip = VideoClip | AudioClip | MusicClip | TitleClip | CaptionClip

Each variant carries only the fields it actually needs. The discriminant is `type: "video" | "audio" | "music" | "title" | "caption"`. Shared base fields (id, startMs, durationMs, etc.) live on a `BaseClip` that each variant extends.
